...
server <- function(input, output){
  output$modelplot <- renderPlot({
    ...
    ### COMIENZA 'codes/2_simOutbreak_ncov_SEIcIscR_ARG.r' ###
    
    set.seed(123)
  
    
    epi_doNothing_l = vector('list', nsim)
    epi_base_l = vector('list', nsim)
    epi_march_l = vector('list', nsim)
    epi_april_l = vector('list', nsim)
